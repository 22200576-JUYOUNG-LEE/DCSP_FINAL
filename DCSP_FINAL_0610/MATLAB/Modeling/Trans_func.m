...
%% 3.2. Phase only Fitting (1st order, lsqcurvefit)

phase_model_1st = @(bw,w) -atan2(w,bw);

[~, idx45] = min(abs(p_exp(idx) + 45));
...
[num_inv, den_inv] = invfreqs(h, w, 0, 1);
sys_inv = tf(num_inv, den_inv);

%% 5. 최종 시스템 선택 — phase fitting 기준
sys_final = sys_phase;

sys_final_1st=sys_phase_1st;

%% 6. Report
fprintf('\n--- Phase Fitting Result_2nd ---\n');
fprintf('wn_2nd   = %.4f rad/s = %.4f Hz\n', wn_est, fn_est);
fprintf('zeta_2nd = %.4f\n', zeta_est);
fprintf('DC gain_2nd = %.4f\n', g_initial);
fprintf('\n--- invfreqs Result (reference) ---\n');
printsys(num_inv, den_inv, 's');


fprintf('\n--- Phase Fitting Result_1nd ---\n');
fprintf('wn_1st   = %.4f rad/s \n', pm_est);
fprintf('DC gain_1nd = %.4f\n', g_initial);
fprintf('\n--- invfreqs Result (reference) ---\n');
printsys(num_inv, den_inv, 's');
...
semilogx(f_exp, 20*log10(g_norm), ...
    'k.', 'MarkerSize', 12, 'DisplayName', 'Measured');

[m_phase, ~] = bode(sys_phase, 2*pi*f_exp);
semilogx(f_exp, 20*log10(squeeze(m_phase)), ...
    'r-', 'LineWidth', 2, ...
    'DisplayName', sprintf('2nd-order fit: fn=%.2fHz, zeta=%.3f', fn_est, zeta_est));

[m_phase_1st, ~] = bode(sys_phase_1st, 2*pi*f_exp);
semilogx(f_exp, 20*log10(squeeze(m_phase_1st)), ...
    'g-.', 'LineWidth', 2, ...
    'DisplayName', sprintf('1st-order fit: bw=%.2f rad/s', pm_est));

[m_inv, ~] = bode(sys_inv, 2*pi*f_exp);
semilogx(f_exp, 20*log10(squeeze(m_inv)), ...
    'b--', 'LineWidth', 1.5, ...
    'DisplayName', 'invfreqs');

ylabel('Magnitude (dB)');
title('Experimental vs Fitted');
legend('Location','southwest');
set(gca,'XScale','log');
xlim([f_exp(1), f_exp(end)]);
...
semilogx(f_exp, p_exp, ...
    'k.', 'MarkerSize', 12, 'DisplayName', 'Measured');

[~, p_phase] = bode(sys_phase, 2*pi*f_exp);
semilogx(f_exp, squeeze(p_phase), ...
    'r-', 'LineWidth', 2, ...
    'DisplayName', '2nd-order fit');

[~, p_phase_1st] = bode(sys_phase_1st, 2*pi*f_exp);
semilogx(f_exp, squeeze(p_phase_1st), ...
    'g-.', 'LineWidth', 2, ...
    'DisplayName', '1st-order fit');

[~, p_inv] = bode(sys_inv, 2*pi*f_exp);
semilogx(f_exp, squeeze(p_inv), ...
    'b--', 'LineWidth', 1.5, ...
    'DisplayName', 'invfreqs');
